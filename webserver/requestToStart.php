...
$query=mysql_query("SELECT Tools.toolName, Tools.description, Tools.tid, Permissions.count, Permissions.uid FROM Permissions join Tools on Permissions.tid=Tools.tid join Users on Permissions.uid=Users.uid WHERE Users.username='$username' AND Users.password='$password' AND Tools.nfcid='$nfcid'");
if($query)	{
	...
	if(mysql_num_rows($query)==1)	{
		...
		$count = $result[3]+1;
		mysql_query("UPDATE Permissions SET count='$count' WHERE tid='$result[2]' and uid = '$result[4]'");
	}
	else	{
